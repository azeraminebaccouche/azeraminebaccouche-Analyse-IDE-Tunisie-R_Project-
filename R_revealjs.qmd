...
p2 <- ggplot(stats_data, aes(x = "", y = Total_Jobs)) +
  geom_boxplot(fill = "orange", alpha = 0.5, outlier.color = "red", outlier.size = 3) +
  geom_jitter(width = 0.2, color = "darkblue") + # Points réels
  theme_minimal() +
  labs(
    title = "Distribution des Emplois (Détection d'Outliers)",
    y = "Emplois"
  )
print(p2)
```
